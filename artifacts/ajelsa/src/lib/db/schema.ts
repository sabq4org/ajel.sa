/**
 * Ajel Newspaper — Database Schema (ajelsa-local copy)
 *
 * NOTE: this file MUST stay in sync with `lib/db/src/schema/index.ts`,
 * which is the shared schema consumed by `artifacts/api-server`.
 *
 * The two files exist in parallel because ajelsa is currently pinned to
 * drizzle-orm ^0.38.3 while the shared `@workspace/db` package uses the
 * catalog drizzle-orm ^0.45.2 — mixing the two would cause type and
 * runtime incompatibilities. They will be unified once ajelsa upgrades
 * to the catalog drizzle version (planned as part of the
 * Vercel/Railway split).
 *
 * Any change here must be mirrored in `lib/db/src/schema/index.ts`.
 */
