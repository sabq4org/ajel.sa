 ...
 * (not `*`).
 *
 * Pass `null` to clear (per-request `init.credentials` still wins).
 */
export function setCredentials(credentials: RequestCredentials | null): void {
...
export async function customFetch<T = unknown>(
  ...
  const credentials = init.credentials ?? _credentials ?? undefined;
